...
class _ResumeRegionState extends State<ResumeRegion> {
  ...
  void _updateRegionSelection(String selectedRegion) {
    ...
    List<String> updatedRegions = List<String>.from(provider.regions);

    if (updatedRegions.isEmpty) {
      updatedRegions.add(selectedRegion);
    } else {
      updatedRegions[0] = selectedRegion;
    }
          ...
          child: ListView(
            children: subRegions.map((subRegion) {
              return ListTile(
                ...
                onTap: () {
                  String selectedRegion = '$mainRegion $subRegion';
                  _updateRegionPlusSelection(selectedRegion);
                  Navigator.pop(context); // 현재 하단 시트 닫기
                },
              );
            }).toList(),
  ...
  void _updateRegionPlusSelection(String selectedRegion) {
    regionplusController.text = selectedRegion;
    var provider = Provider.of<ResumeWriteProvider>(context, listen: false);
    List<String> updatedRegions = List<String>.from(provider.regions);
    ...
    updatedRegions[1] = selectedRegion;
  ...
  void regionplusbottomsheet(String region, BuildContext context) {
    switch (region) {
      case '서울':
        _showSubRegionPlusModal(context, '서울', subseoul);
        break;
      case '부산':
        _showSubRegionPlusModal(context, '부산', subbusan);
        break;
      case '대구':
        _showSubRegionPlusModal(context, '대구', subdaegu);
        break;
      case '인천':
        _showSubRegionPlusModal(context, '인천', subincheon);
        break;
      case '광주':
        _showSubRegionPlusModal(context, '광주', subgwangju);
        break;
      case '대전':
        _showSubRegionPlusModal(context, '대전', subdaejeon);
        break;
      case '울산':
        _showSubRegionPlusModal(context, '울산', subulsan);
        break;
      case '경기도':
        _showSubRegionPlusModal(context, '경기도', subgyeonggi);
        break;
      case '강원도':
        _showSubRegionPlusModal(context, '강원도', subgangwon);
        break;
      case '충청북도':
        _showSubRegionPlusModal(context, '충청북도', subchungbuk);
        break;
      case '충청남도':
        _showSubRegionPlusModal(context, '충청남도', subchungnam);
        break;
      case '전라북도':
        _showSubRegionPlusModal(context, '전라북도', subjeonbuk);
        break;
      case '전라남도':
        _showSubRegionPlusModal(context, '전라남도', subjeonnam);
        break;
      case '경상북도':
        _showSubRegionPlusModal(context, '경상북도', subgyeongbuk);
        break;
      case '경상남도':
        _showSubRegionPlusModal(context, '경상남도', subgyeongnam);
        break;
      case '제주도':
        _showSubRegionPlusModal(context, '제주도', subjeju);
        break;
      default:
        _updateRegionPlusSelection(region);
    }
  }
}
